            ...
            <div className="mt-8 bg-white dark:bg-navy-900 p-6 rounded-xl border border-gray-200 dark:border-white/10 shadow-sm">
                ...
                <div className="flex gap-4">
                    <input
                        ...
                        className="flex-1 px-4 py-2 rounded-lg border border-gray-300 dark:border-white/20 bg-transparent text-navy-950 dark:text-white focus:ring-2 focus:ring-purple-500 outline-none"
                    />
                    <button
                        onClick={handleIngest}
                        disabled={isIngesting || !ingestUrl}
                        className="px-6 py-2 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-medium rounded-lg transition-colors flex items-center gap-2"
                    >
                        {isIngesting ? (
                            <>
                ...
                {ingestResult && (
                    <div className="mt-6 animate-in fade-in slide-in-from-top-4">
                        <div className={`p-4 rounded-lg border ${ingestResult.isRelevant ? 'bg-green-50 border-green-200 dark:bg-green-900/10 dark:border-green-900/30' : 'bg-gray-50 border-gray-200 dark:bg-gray-800 dark:border-gray-700'}`}>
                            <div className="flex items-start gap-3">
                                <div className={`mt-1 p-1.5 rounded-full ${ingestResult.isRelevant ? 'bg-green-100 text-green-600' : 'bg-gray-200 text-gray-500'}`}>
                                    {ingestResult.isRelevant ? (
                                ...
                                <div className="flex-1">
                                    <h4 className={`font-bold ${ingestResult.isRelevant ? 'text-green-800 dark:text-green-200' : 'text-gray-800 dark:text-gray-200'}`}>
                                        {ingestResult.isRelevant ? 'Relevant Insight Detected' : 'Content Filtered (Low Relevance)'}
                                    </h4>
                                    <p className="mt-1 text-sm text-gray-700 dark:text-gray-300">
                                        {ingestResult.summary}
                                    </p>
                                    <div className="mt-3 flex flex-wrap gap-2">
                                        {ingestResult.tags?.map((tag: string) => (
                                            <span key={tag} className="px-2 py-1 text-xs font-medium bg-white dark:bg-black/20 rounded border border-gray-200 dark:border-white/10">
                                                #{tag}
                                            </span>
                                        ))}
                                    </div>
                                    <div className="mt-2 text-xs text-gray-500 italic">
                                        Reasoning: {ingestResult.relevanceReasoning}
                                    </div>
                                </div>
                            </div>
                        </div>
